...
pub fn round_trip_full_model_test() {
  let original =
    HydrationModel(
      ...
      plays: sample_plays(),
      repos: sample_repos(),
      rewrites: dict.from_list([
        #("https://cdn.example/avatar.jpg", "/img/profile/avatar.jpeg"),
      ]),
    )
  ...
  decoded.rewrites |> should.equal(original.rewrites)
}
...
pub fn round_trip_empty_lists_test() {
  ...
  decoded.rewrites |> should.equal(dict.new())
}

pub fn script_safe_escaping_test() {
  let hostile =
    ProfileViewDetailed(
      did: "did:plc:xss",
      handle: "xss.test",
      display_name: Some("</script><script>alert(1)</script>"),
      description: Some("a & b < c > d"),
      avatar: Some("https://example.com/avatar.jpg"),
      banner: Some("https://example.com/banner.jpg"),
      followers_count: Some(1),
      follows_count: Some(1),
      posts_count: Some(1),
      pronouns: None,
    )
  let original =
    HydrationModel(profile: hostile, plays: [], repos: [], rewrites: dict.new())

  let json = encode.encode_hydration_model(original)
  // Raw `</script>` would terminate the embedding script tag.
  json |> string.contains("</script>") |> should.be_false()
  json |> string.contains("\\u003c/script\\u003e") |> should.be_true()
  // The escapes are lossless for the JSON decoder.
  let assert Ok(decoded) = decode.decode_hydration_model(json)
  decoded.profile |> should.equal(hostile)
}
...
pub fn round_trip_zero_counts_test() {
  ...
  let original =
    HydrationModel(profile: profile, plays: [], repos: [], rewrites: dict.new())
  let json = encode.encode_hydration_model(original)
  let assert Ok(decoded) = decode.decode_hydration_model(json)
