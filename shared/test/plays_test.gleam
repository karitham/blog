...
pub fn plays_rows_renders_rows_test() {
  let assert Ok(#(decoded, _drops)) = atproto.decode_plays(sample_play_body())
  let html = plays.plays_rows(decoded, True) |> to_string
  string.contains(html, "Track A") |> should.be_true()
  string.contains(html, "Artist A") |> should.be_true()
}

pub fn plays_rows_stale_sets_attribute_test() {
  let assert Ok(#(decoded, _drops)) = atproto.decode_plays(sample_play_body())
  plays.plays_rows(decoded, True)
  |> to_string
  |> string.contains("data-stale")
  |> should.be_true()
}

pub fn plays_rows_fresh_omits_attribute_test() {
  let assert Ok(#(decoded, _drops)) = atproto.decode_plays(sample_play_body())
  plays.plays_rows(decoded, False)
  |> to_string
  |> string.contains("data-stale")
  |> should.be_false()
}
